                ...
                <table class="w-full bg-white border border-gray-300">
                    <thead>
                        <tr>
                            ...
                            <th class="py-2 px-4 border-b">Ghi chú</th>
                            <th class="py-2 px-4 border-b">Trang thái</th>
                    ...
                    <tbody>
                        @foreach ($cooperateData as $id=> $data )
                            <tr>
                                <td class="py-2 px-4 border-b">{{ $id+1}}</td>
                                <td class="py-2 px-4 border-b">{{ $data->name }}</td>
                                <td class="py-2 px-4 border-b">{{ $data->email }}</td>
                                <td class="py-2 px-4 border-b">{{ $data->sdt }}</td>
                                <td class="py-2 px-4 border-b">{{ $data->note }}</td>
                              @if ($data->status=0)
                              <td class="py-2 px-4 border-b">Chưa duyệt</td>
                              @else
                              <td class="py-2 px-4 border-b">Đã duyệt</td>
                              @endif
                                <td class="py-2 px-4 border-b">{{ $data->created_at->format('d-m-Y')}}</td>
                                
                                <td class="py-2 px-4 border-b">
                                    <div>
                                        <div>
                                           
                                            <form action="{{ route('admincontactupdate', ['id' => $data->id]) }}" method="POST" class="inline-block">
                                                @csrf
                                                @method('POST')
                                                <button type="submit" class="bg-green-500 hover:bg-green-700 text-white py-1 px-2 rounded-full mr-2">Duyệt</button>
                                            </form>
                                            <form action="{{ route('admincontactdelete', ['id' => $data->id]) }}" method="POST" class="inline-block">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="bg-red-500 hover:bg-red-700 text-white py-1 px-2 rounded-full">Xóa</button>
                                            </form>
                                        </div>
                                    </div>
                                </td>
