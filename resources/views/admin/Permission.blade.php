    ...
    <div class="min-h-screen flex flex-col flex-auto flex-shrink-0 antialiased text-black ">
        ...
        <div class="h-full ml-14 mt-14 mb-10 md:ml-64">
            ...
            <div class="mt-4 mx-4">
                ...
                <form id="addGroupForm" action="{{ route('adminpermissions') }}" method="post">
                    @csrf
                    <div class="flex items-center my-4">
                        <input type="text" name="name" placeholder="Nhập tên group mới..."
                ...
                <form id="myForm" method="POST" action="{{ route('adminpermissionput') }}">
                    @csrf
                    @method('PUT')

                    <div class="mb-5">
                        <label for="gr" class="text-sm font-medium text-gray-700">Chọn nhóm:</label>
                        <select class="select select-bordered w-full max-w-xs" id="gr" name="group">
                            <option value="Group ID">Group ID</option>
                            @foreach ($data['group'] as $item)
                                <option value="{{ $item->id }}" data-permission="{{ $item->permission }}">
                                    {{ $item->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <button type="button" onclick="submitForm()">Chọn</button>
                </form>
        ...
        <script>
            document.getElementById('gr').addEventListener('change', function() {
                // Lấy giá trị của data-permission từ option đã chọn
                var selectedPermission = this.options[this.selectedIndex].getAttribute('data-permission');

                // Gọi hàm để xử lý dữ liệu quyền
                handlePermissionData(selectedPermission);
            });

            function handlePermissionData(permission) {
                // Ở đây, bạn có thể sử dụng giá trị quyền (permission) theo nhu cầu của mình
                console.log('Selected Permission:', permission);

                // Ví dụ: cập nhật trạng thái checkbox dựa trên quyền
                updateCheckboxState(permission);
            }

            function updateCheckboxState(permission) {
                var checkboxes = document.getElementsByName('items[]');

                checkboxes.forEach(function(checkbox) {
                    // Kiểm tra xem quyền của checkbox có trong danh sách quyền không
                    var hasPermission = permission.includes(checkbox.value);
