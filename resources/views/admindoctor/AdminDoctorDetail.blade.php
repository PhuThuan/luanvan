...
<body>
    <div class="min-h-screen flex flex-col flex-auto flex-shrink-0 antialiased text-black ">
        ...
        <div class="h-full ml-14 mt-14 mb-10 md:ml-64">
            ...
            <div class="mt-4 mx-4">
                <h2 class="text-2xl font-bold mb-4">Lịch làm việc của {{ $nameDoctor }}</h2>
                <form id="roleForm" action="{{ route('admindcotorchangegroup', ['user_id' => $dataResult['user_id']]) }}" method="POST">
                    @csrf <!-- Laravel CSRF protection token -->
                
                    <div class="...">
                        <select class="select select-bordered w-full max-w-xs mb-5" id="rl" name="selectedRole">
                            <option selected>Quyền của bác sĩ: {{ $dataResult['currentRole'] }}</option>
                            @foreach ($dataResult['role'] as $index => $role)
                                <option value="{{ $role }}">{{ $index }} - {{ $role }}</option>
                            @endforeach
                        </select>
                    </div>
                </form>
                <div class="flex justify-between mb-4">
                    <a href="{{ route('adminsdoctordetail', ['id' => $idDoctor, 'day' => $idRoute - 1]) }}"> <button
            ...
            <div class="bg-white rounded-lg shadow-lg ">
                ...
                <div class="bg-white rounded-lg shadow-lg p-4">
                    ...
                    <div class="info flex justify-between">
                        <div>
                            <p><strong>Ngày bắt đầu tuần:</strong> {{ $startOfWeek }}</p>
                        </div>
                        <div class="info">

                            <p class="text-center"><strong>Ngày hiện tại:</strong> {{ $today }}</p>
                            <p class="text-center"><strong>Thứ hiện tại:</strong> {{ $todayN }}</p>
                        </div>
                        <div>
                            <p><strong>Ngày kết thúc tuần:</strong> {{ $endOfWeek }}</p>
                        </div>
                ...
                <div class="mt-6">
                    ...
                    <!-- Replace the placeholders with your data for each day -->
