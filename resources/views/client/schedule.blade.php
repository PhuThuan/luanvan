...
<body>
    ...
    <div class="   pl-[13%] border border-gray-200 bg-blue-50 ">
        ...
        <div class="mb-4    ml-[10%] w-[80%]">
            <label for="ten_benh_vien" class="block text-gray-700 text-sm font-bold mb-2">Tên phòng khám:</label>
        ...
        <div class="mt-6   pl-[5%] w-[90%]">

            <div id="items" class="flex">
                <ul class="w-[60%] overflow-y-auto" id="hospitalList">
                    @foreach ($hospitals as $id => $hospital)
                        <li class="p-4 border border-gray-200" data-hospital-id="{{ $id }}">
                            <div class="flex">
                                <div class="w-1/5 mt-[5%] ml-[5%]">
                ...
                <div class="details-container w-[40%] p-4">
    ...
    <div class=" bg-cover bg-center relative    pl-[13%] "
        ...
        <div class="text-center pr-[10%]">
            <form action="{{ route('contact') }}" class="pl-[50%] h-[50%] pt-[3%]" method="POST">
                ...
                <div class="mb-4">
                    <input type="text" id="name" name="name" placeholder="Tên đơn vị/ Người liên hệ"
                        required
   ...
   $(document).ready(function () {
    let id = 0; // Initialize id to 1 by default

    // Call showDetails with the default id
    showDetails(id);

    $('#hospitalList').on('click', 'li', function () {
        const clickedId = $(this).data('hospital-id');
        
        // Update id only if it's different from the default (1)
    ...
    function showDetails(id) {
        // Lấy thông tin chi tiết của bệnh viện được chọn
        console.log(id);

        const selectedHospital = {!! json_encode($hospitals) !!}[id];

        // Check if selectedHospital and id property exist
        if (selectedHospital && selectedHospital.hasOwnProperty('id')) {
            console.log('Clicked!', selectedHospital.id);

            // Hiển thị thông tin chi tiết ở bên phải
            $('.details-container').html(`
                <img src="${selectedHospital.logo}" alt="Logo Bệnh Viện" class="w-[50%] h-[50%] ml-[25%]">
                <h2 class="text-xl font-semibold">${selectedHospital.name}</h2>
                <p class="text-gray-600">${selectedHospital.id_address}</p>
                <p class="text-gray-600">${selectedHospital.introduce}</p>
            `);
        } else {
            console.error('Invalid hospital data or missing id property.');
        }
    }
});
